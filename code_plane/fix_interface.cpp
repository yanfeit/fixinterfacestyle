  ...
  nwall(0)
{
  ...
  while (iarg < narg) {
    if ((strcmp(arg[iarg],"xlo") == 0) || (strcmp(arg[iarg],"xhi") == 0) ||
      ...
      if (strstr(arg[iarg+2],"v_") == arg[iarg+2]) {
        ...
        gamma0[nwall] = force->numeric(FLERR,arg[iarg+2]);
      ...
      if (strstr(arg[iarg+4],"v_") == arg[iarg+4]) {
        ...
        sstr[nwall] = new char[n];
        strcpy(sstr[nwall],&arg[iarg+4][2]);
        sstyle[nwall] = VARIABLE;
      } else {
        costheta[nwall] = force->numeric(FLERR,arg[iarg+4]);
        sstyle[nwall] = CONSTANT;
      }
  ...
  for (int m = 0; m < nwall; m++)
    if ( (costheta[m] > 1.0) || (costheta[m] < -1.0) )
      error->all(FLERR,"Invalid Contact angle!");
